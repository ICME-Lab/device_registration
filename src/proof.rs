...
use nova::{nebula::rs::{PublicParams, RecursiveSNARK}, onchain::compressed::{CompressedPK, CompressedSNARK, CompressedVK}, provider::{Bn256EngineKZG, GrumpkinEngine}, traits::{snark::RelaxedR1CSSNARKTrait, Engine}};
...
pub fn setup_decider(rs_pp: PublicParams<E1>, rng: &mut ThreadRng) -> (CompressedPK, CompressedVK) {
    let start = Instant::now();
    let (decider_pk, decider_vk) = CompressedSNARK::setup(&rs_pp, rng, 1).unwrap();
    ...
    (decider_pk, decider_vk)
}

pub fn generate_decider_proof(rs: RecursiveSNARK<E1>, rs_pp: PublicParams<E1>, decider_pk: CompressedPK, rng: &mut ThreadRng) -> CompressedSNARK {
    // produce a compressed SNARK
    let start = Instant::now();
    let res = CompressedSNARK::prove(&rs_pp, &decider_pk, &rs, rng);
    assert!(res.is_ok());
    let compressed_snark = res.unwrap();
    println!("Decider prove took: {:?}", start.elapsed());
    save_to_bson(&compressed_snark, "compressed_snark.bson");
    // let compressed_snark = load_from_bson::<CompressedSNARK>("compressed_snark.bson");
    compressed_snark
}

pub fn sign_proof_with_device(compressed_snark: &CompressedSNARK) -> ([u8; 32], Signature) {
    // compute proof hash
...
pub fn verify_proof(compressed_snark: &CompressedSNARK, compressed_vk: &CompressedVK) -> bool {
    let start = Instant::now();
    let res = compressed_snark.verify(compressed_vk.clone());
